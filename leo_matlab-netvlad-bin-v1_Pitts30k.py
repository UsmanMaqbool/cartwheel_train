...
def db247():
    ...
    base_Addr = "/app/datasets/NetvLad/Pittsburgh/images"
    # dbTest_ImageFns = sio.loadmat('/app/datasets/NetvLad/view-tags/dbPitts30kImageFns.mat')
    dbTest_ImageFns = sio.loadmat('/app/datasets/NetvLad/view-tags/qPitts30kImageFns.mat')


    lst_images = [];
    for index in range(0,len(dbTest_ImageFns['aa'][0][0])):
        lst_images.append(dbTest_ImageFns['aa'][0][0][index][0][0].encode("ascii")); 

    feat_mat = np.zeros((len(lst_images),8192));
    i = 0;

    for img in lst_images:

  #  for filename in glob.glob(dataset_DIR+'/*.jpg'):
        filename = os.path.join(base_Addr,img)
        print filename
       # im = cv2.imread(filename,0)
        ...
        feat_mat[i,:] = y;
        i += 1;
    return feat_mat    #(216, 4096) (total 216 images, each has 4096 lenght of descriptor)
...
if __name__ == '__main__':
    yy = db247()    # len(yy) =  216
    # f_name = "/app/docker_ws/netvlad/netvlad-orignal/output/dbPitt30k_GSVFns_db.bin"
    f_name = "/app/docker_ws/netvlad/netvlad-orignal/output/qPitt30k_GSVFns_q.bin"
 
    fea_fc7_bin = yy.flatten() # len(xy) = 884736
    savefile(fea_fc7_bin,f_name)
